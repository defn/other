// Package gmailbot generates Gmail bot instance files from the catalog.
package gmailbot
...
// Run generates BUILD.bazel, mise.toml, .gitignore for each Gmail bot.
func Run(ctx *gen.Context) error {
	bots := ctx.CatalogQuery("gmail_bots")
	...
	if err := gen.IterMap(bots, func(key string, val cue.Value) error {
		...
		return fmt.Errorf("iterate gmail_bots: %w", err)
	}
	...
	for _, e := range entries {
		if err := ctx.StampFromCUE(
			"kernel/interface/gmail-bot/templates.cue", e.path,
			map[string]string{
				"name":         e.name,
