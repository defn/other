...
func (r *JobRunner) createEnvironment(ctx context.Context) ([]string, error) {
	...
	if pluginsJSON := env["BUILDKITE_PLUGINS"]; pluginsJSON != "" && r.conf.KubernetesExec {
		...
		if err != nil {
			r.agentLogger.Error("Invalid BUILDKITE_PLUGINS: %v", err)
		}
		if string(filtered) == "" {
