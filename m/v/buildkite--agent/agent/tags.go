...
func (t *tagFetcher) Fetch(ctx context.Context, l logger.Logger, conf FetchTagsConfig) []string {
	...
	if conf.TagsFromEC2MetaData {
		...
		if err != nil {
			l.Error("Failed to fetch EC2 meta-data: %s", err.Error())
		}
	}
	...
	if len(conf.TagsFromEC2MetaDataPaths) > 0 {
		...
		if err != nil {
			l.Error("Error parsing meta-data tag and path pairs: %s", err.Error())
		}
	...
	if conf.TagsFromEC2Tags {
		...
		// Don't blow up if we can't find them, just show a nasty error.
		if err != nil {
			l.Error("Failed to find EC2 tags: %s", err.Error())
		}
	}
	...
	if conf.TagsFromECSMetaData {
		...
		// Don't blow up if we can't find them, just show a nasty error.
		if err != nil {
			l.Error("Failed to fetch ECS meta-data: %s", err.Error())
		}
	}
	...
	if conf.TagsFromGCPMetaData {
		...
		err := roko.NewRetrier(
			...
			if err != nil {
				// Don't blow up if we can't find them, just show a nasty error.
				l.Error("Failed to fetch Google Cloud meta-data: %s", err.Error())
				return err
			}
		...
		// Don't blow up if we can't find them, just show a nasty error.
		if err != nil {
			l.Error("Failed to fetch GCP meta-data: %s", err.Error())
		}
	}
	...
	if conf.TagsFromGCPLabels {
		...
		// Don't blow up if we can't find them, just show a nasty error.
		if err != nil {
			l.Error("Failed to find GCP instance labels: %s", err.Error())
		}
	}
