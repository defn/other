...
func NewWrapper(opts ...WrapperOpt) (*Wrapper, error) {
	...
	//
	// But if the shebang specifies a non-shell interpreter (e.g. python)
	// the wrapper won't work. Non-shell hooks should never be wrapped.
...
func WriteHookWrapper(
	...
	// Sync to ensure the write is flushed to disk before exec.
	// Without this, parallel tests can hit ETXTBSY on Linux.
