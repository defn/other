// Package gitstore is the performance-sensitive git wrapper for artifact-fs.
// Most of the easy-to-break invariants live here.
//
// Constraints to preserve:
//
//   - Credentials must NEVER appear in git CLI args. Use the env-based
//     credential helper path so tokens do not show up in `ps`. Keep log
//     output redacted with auth.RedactString.
//   - Blob content must stay binary-safe. Do NOT convert blob bytes to
//     `string`. BlobToCache streams `git cat-file --batch` output to disk
//     for a reason.
//   - Keep GIT_NO_LAZY_FETCH=1 on `git cat-file --batch-check` in
//     batchResolveSizes. Without it, blobless clones turn size resolution
//     into network round-trips.
//   - git.CloneBlobless already populates the git index with `read-tree HEAD`.
//     Be careful about extra index resets -- they can discard staged state.
package gitstore
...
func (s *Store) batchResolveSizes(ctx context.Context, repo model.RepoConfig, nodes []model.BaseNode, oids []string, index map[string][]int) error {
	...
	//
	// NOTE: on git 2.32+, GIT_NO_LAZY_FETCH=1 makes cat-file fatal-exit
	// (status 128) on the first missing blob instead of emitting
	// "<oid> missing" -- so on a fresh blobless clone this path always
	// fails and every row stays size_state="unknown". The warning below
	// will include git's stderr so the actual error is visible in logs.
