...
// Config holds configuration for the go-cmd-cue stamp subcommand.
type Config struct {
	Path string
	Desc string
}

// Service implements ServiceRunner for stamping go-cmd-cue bricks.
type Service struct{}

// NewService creates a new service.
func NewService() *Service { return &Service{} }

// Run stamps a go-cmd-cue brick.
func (s *Service) Run(_ context.Context, cfg Config, onReady func(error)) error {
	onReady(nil)
	rootDir, _ := os.Getwd()
	return stamplib.StampBrick(rootDir, "go-cmd-cue", cfg.Path, cfg.Desc)
}
