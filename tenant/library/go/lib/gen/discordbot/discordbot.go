// Package discordbot generates Discord bot instance files from the catalog.
package discordbot
...
// Run generates BUILD.bazel, mise.toml, .gitignore for each Discord bot.
func Run(ctx *gen.Context) error {
	bots := ctx.CatalogQuery("discord_bots")
	...
	if err := gen.IterMap(bots, func(key string, val cue.Value) error {
		...
		return fmt.Errorf("iterate discord_bots: %w", err)
	}
	...
	for _, e := range entries {
		if err := ctx.StampFromCUE(
			"kernel/interface/discord-bot/templates.cue", e.path,
			map[string]string{
				"name":         e.name,
