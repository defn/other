// Package matrixbot generates Matrix bot instance files from the catalog.
package matrixbot
...
// Run generates BUILD.bazel, mise.toml, .gitignore for each Matrix bot.
func Run(ctx *gen.Context) error {
	bots := ctx.CatalogQuery("matrix_bots")
	...
	if err := gen.IterMap(bots, func(key string, val cue.Value) error {
		...
		return fmt.Errorf("iterate matrix_bots: %w", err)
	}
	...
	for _, e := range entries {
		if err := ctx.StampFromCUE(
			"kernel/interface/matrix-bot/templates.cue", e.path,
			map[string]string{
				"name":         e.name,
