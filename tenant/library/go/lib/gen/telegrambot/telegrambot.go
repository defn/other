// Package telegrambot generates TelegramBot instance files from the catalog.
package telegrambot
...
// Run generates BUILD.bazel, mise.toml, .gitignore for each TelegramBot.
func Run(ctx *gen.Context) error {
	bots := ctx.CatalogQuery("telegram_bots")
	...
	if err := gen.IterMap(bots, func(key string, val cue.Value) error {
		...
		return fmt.Errorf("iterate telegram_bots: %w", err)
	}
	...
	for _, e := range entries {
		if err := ctx.StampFromCUE(
			"kernel/interface/telegram-bot/templates.cue", e.path,
			map[string]string{
				"name":         e.name,
